  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description="Проверки наличия, перемещений и повреждений оборудования."
      />
      {canManageInventory && (
        <ContentCard>
          <form onSubmit={create} className="flex max-w-3xl flex-col gap-3 sm:flex-row">
            <input className="input" placeholder="Название проверки" value={title} onChange={(e) => setTitle(e.target.value)} required />
            <button className="btn-primary shrink-0"><Plus className="h-4 w-4" weight="regular" /> Создать</button>
          </form>
        </ContentCard>
      )}
          ...
          <ScrollArea className="max-h-[70vh] space-y-2 p-4 pr-2">
            {checks.map((check) => (
              <button key={check.id} onClick={() => open(check.id)} className="w-full rounded-md border border-surface-200 p-3 text-left transition hover:border-primary-400/35 hover:bg-surface-200/50">
                ...
                <p className="mt-1 text-xs text-surface-500">Позиций: {check._count?.items || 0}</p>
              </button>
            ))}
          </ScrollArea>
        </ContentCard>
        <ContentCard className="min-h-0">
          {!selected ? <p className="text-surface-500">Выберите проверку слева.</p> : (
            <>
              <div className="mb-4 flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  <h2 className="truncate text-lg font-extrabold">{selected.title}</h2>
                  <p className="text-xs text-surface-500">{new Date(selected.startedAt).toLocaleDateString('ru-RU')}</p>
                </div>
              ...
              {canManageInventory && selected.status !== 'COMPLETED' && (
                <form onSubmit={addItem} className="mb-4 rounded-md border border-surface-200 p-3 space-y-3">
                  <p className="text-sm font-extrabold">Добавить оборудование в проверку</p>
                  <div className="grid grid-cols-1 gap-2 lg:grid-cols-2 2xl:grid-cols-4">
                    ...
                    <select className="input" value={newItem.expectedLocationId} onChange={(e) => setNewItem({ ...newItem, expectedLocationId: e.target.value })}>
                      <option value="">Ожидаемая локация</option>
                    ...
                    <select className="input" value={newItem.actualLocationId} onChange={(e) => setNewItem({ ...newItem, actualLocationId: e.target.value })}>
                      <option value="">Фактическая локация</option>
                      {locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
                    </select>
                    <select className="input" value={newItem.status} onChange={(e) => setNewItem({ ...newItem, status: e.target.value })}>
                      {['FOUND', 'MISSING', 'MOVED', 'DAMAGED'].map((s) => <option key={s}>{s}</option>)}
                    </select>
                  </div>
                  <button className="btn-primary">Добавить позицию</button>
                </form>
              )}
              <ScrollArea className="max-h-[60vh] space-y-2">
                {selected.items.map((item: any) => <div key={item.id} className="rounded-md border border-surface-200 p-3">
                  ...
                  <p className="mt-2 break-words text-xs text-surface-500">Ожидалось: {item.expectedLocation?.name || '—'} · Фактически: {item.actualLocation?.name || '—'}</p>
                  {canManageInventory && <div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">{['FOUND', 'MISSING', 'MOVED', 'DAMAGED'].map((s) => <button key={s} className="btn-secondary px-3 py-1 text-xs" onClick={() => updateItem(item.id, s)}>{s}</button>)}</div>}
                </div>)}
              </ScrollArea>
              {canManageInventory && selected.status !== 'COMPLETED' && <button className="btn-success mt-4" onClick={complete}>Завершить проверку</button>}
            </>
          )}
        </ContentCard>
      </div>
